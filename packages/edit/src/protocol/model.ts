...
import { createDelOperation, createUpdateOperation } from './operation';
...
export class Model {
  ...
  setModel(_model) {
    this.normalize = this.normalize.bind(this);
    this._model = this.normalize(_model);
    this.editor.eventCenter.emit('model-change', this.editor.model);
  }

  onChange(cb) {
    this.editor.eventCenter.on('model-change', cb, this.editor);
  }
  ...
  applyModelChange() {
    ...
    this.timer = setTimeout(() => {
      ...
      this.editor.eventCenter.emit('model-change', editor.model);
      selection.reset();
    }, 0);
  }
  ...
  applyOperation(operation) {
    ...
    if (type === 'delete') {
      const block = editor.getBlockByid(arg.id);
      block.parent.blocks = block.parent.blocks.filter(({ id }) => id !== arg.id);
    }
  ...
  deleteContent(range) {
    ...
    if (startContainer === endContainer) {
      ...
      this.applyOperation(createUpdateOperation(editor, startContainer.id, { text }));
    } else {
  ...
  insertText(data: string) {
    ...
    if (['`', '*', '_', "'", '"'].includes(data) && data !== text[focusOffset - 1]) {
    ...
    this.applyOperation(createUpdateOperation(this.editor, selection.focusBlock.id, { text }));
    console.log(focusOffset, data.length, data, text);
    selection.collapse(focusBlock, focusOffset + data.length);
    this.editor.eventCenter.emit('block-change', focusBlock);
  }
  ...
  insertParagraph(): BlockInterface {
    ...
    this.applyOperation(createUpdateOperation(editor, selection.focusBlock.id, { text: stext }));
